...
class CountdownTimer extends window.HTMLElement {
  ...
  static get observedAttributes () {
    return ['warn-when', 'update-every', 'play-sound', 'blink-colon', 'start-immediately']
  }
  ...
  initializeFromDOM () {
    ...
    const minutes = parseInt(minutesEl?.innerText || '0')
    const seconds = parseInt(secondsEl?.innerText || '0')
    this.duration = minutes * 60 + seconds
    this.display = { minutes, seconds }

    // Initialize properties from attributes
    this.warn_when = parseInt(this.dataset.warnWhen) || -1
    this.update_every = parseInt(this.dataset.updateEvery) || 1
    this.play_sound = this.attrIsTrue(this.dataset.playSound) || this.dataset.playSound
    this.blink_colon = this.attrIsTrue(this.dataset.blinkColon)
    this.startImmediately = this.attrIsTrue(this.dataset.startImmediately)
  ...
  emitStateEvent (action) {
    ...
    this.dispatchEvent(new CustomEvent('countdown', { detail: data, bubbles: true }))
  }
